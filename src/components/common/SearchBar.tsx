...
export default function SearchBar({ value, onChange }: SearchBarProps) {
  ...
  return (
    <div className="sticky top-0 z-50 w-full backdrop-blur-sm px-5 py-3">
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="relative rounded-full border border-[var(--color-border)] shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1),0_4px_6px_-4px_rgba(0,0,0,0.1)] bg-[var(--color-surface)]">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-[18px] bg-transparent rounded-full focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent transition-shadow text-base text-[var(--color-text-secondary)] placeholder:text-[var(--color-text-muted)]"
            />
          </div>
        </form>
      </div>
